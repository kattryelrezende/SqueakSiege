...
document.addEventListener("DOMContentLoaded", function () {
  ...
  const text = "Squeak Siege, o simulador orbital de foguetes";
  let index = 0;

  // Inicializa o cursor visível
  ...
  function typeText() {
    if (index < text.length) {
      ...
      index++;
      setTimeout(typeText, 100); // Ajusta a velocidade de digitação (100ms por caractere)
    } else {
      // Remova o cursor piscante após a digitação completa
    ...
    if (index < text.length) {
      typewriterElement.style.borderRight =
        typewriterElement.style.borderRight === "2px solid white"
          ? "none"
          : "2px solid white";
      setTimeout(blinkCursor, 500); // Alterna a cada 500ms
    }
  }
...
var options = {
  particleColor: "#888",
  background: "#101010", // Cor do fundo
  interactive: true,
  speed: "medium",
  density: "high",
};
